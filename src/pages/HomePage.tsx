...
const greetings = ['今天也要好好照顾自己哦', '想TA的时候，就来记录一下吧', '每一天都在靠近重逢的那天', '记录此刻，等TA回来看', '你的坚持，TA一定能感受到', '用心记录，用爱等待'];
const quickActions: {
  type: RecordType;
  icon: typeof Sunrise;
  label: string;
...
export default function HomePage() {
  ...
  useEffect(() => {
    async function loadData() {
      setLoading(true);
      const [settingsData, recordsData] = await Promise.all([getSettings(), getRecords()]);
  ...
  return <div className="min-h-screen pb-24 bg-gradient-to-b from-background to-secondary/30">
      {/* Header */}
      <motion.div className="p-6 pt-8 safe-area-top" initial={{
      opacity: 0,
      y: -20
    }} animate={{
      opacity: 1,
      y: 0
        ...
        <motion.div className="mt-4 p-4 bg-card rounded-2xl shadow-soft border border-border" whileHover={{
        scale: 1.02
      }}>
          <div className="flex items-center justify-between">
            <div>
              <div className="text-body-s text-muted-foreground">修行路</div>
            ...
            <motion.div className="w-14 h-14 rounded-full bg-secondary flex items-center justify-center" animate={{
            scale: [1, 1.1, 1]
          }} transition={{
            duration: 2,
            repeat: Infinity
        ...
        <motion.p className="mt-4 text-body-l text-center text-foreground" initial={{
        opacity: 0
      }} animate={{
        opacity: 1
      }} transition={{
      ...
      {/* Comments Banner */}
      {commentsWithRecords.length > 0 && <motion.div className="px-6 mb-4" initial={{
      opacity: 0,
      y: 10
    }} animate={{
      opacity: 1,
      ...
      delay: 0.4
          ...
          <div className="p-4 bg-primary/10 rounded-2xl border border-primary/20 cursor-pointer active:bg-primary/15 transition-colors" onClick={() => setShowCommentSheet(true)}>
            <div className="flex items-center gap-3">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  ...
                  {unrepliedCount > 0 && <span className="text-[11px] text-primary">
                      {unrepliedCount} 条未回复
                    </span>}
                </div>
      ...
      <Sheet open={showCommentSheet} onOpenChange={setShowCommentSheet}>
        <SheetContent side="bottom" className="h-[75vh] rounded-t-3xl pb-safe">
          ...
          <div className="overflow-y-auto flex-1 py-3 -mx-2 px-2" style={{
          maxHeight: 'calc(75vh - 80px)'
        }}>
            {commentsWithRecords.length === 0 ? <div className="flex items-center justify-center h-32 text-muted-foreground text-body-s">
                暂无留言
              </div> : <div className="space-y-2">
                {commentsWithRecords.map(comment => {
              const typeInfo = RECORD_TYPE_INFO[comment.recordType as RecordType];
              return <motion.div key={comment.id} className="p-3 bg-card rounded-xl border border-border cursor-pointer active:bg-secondary/50 transition-colors" onClick={() => {
                      ...
                      <div className="flex items-start gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          minute: '2-digit'
                            ...
                            {comment.hasReply ? <span className="inline-flex items-center gap-0.5 text-[10px] text-muted-foreground bg-secondary px-1.5 py-0.5 rounded-full">
                                <Reply size={9} /> 已回复
                              </span> : <span className="text-[10px] text-primary bg-primary/10 px-1.5 py-0.5 rounded-full font-medium">
                                未回复
                              </span>}
                          </div>
                        </div>
      ...
      <div className="px-6">
        ...
        <div className="grid grid-cols-5 gap-2">
          {quickActions.map((action, index) => <motion.button key={action.type} onClick={() => handleQuickAction(action.type)} className={`flex flex-col items-center p-3 rounded-2xl bg-gradient-to-br ${action.gradient}`} initial={{
          opacity: 0,
          y: 20
        }} animate={{
          opacity: 1,
      ...
      <div className="px-6 mt-8">
        <div className="flex items-center justify-between mb-3">
          ...
          {todayRecords.length > 0 && <span className="text-body-s text-muted-foreground">
              {todayRecords.length} 条
            </span>}
        </div>

        {todayRecords.length === 0 ? <EmptyState icon={Plus} title="还没有今天的记录" description="点击上方的按钮，记录此刻的生活吧" /> : <div className="space-y-3">
            {todayRecords.map(record => <RecordCard key={record.id} record={record} compact onClick={() => navigate(`/edit/${record.id}`)} />)}
          </div>}
      </div>
    </div>;
}